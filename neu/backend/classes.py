...
class Entry:
    ...
    def return_earliest(self, average: int):
        history = yf.Ticker(self.ticker).history(period = "max")
        if history.shape[0] <= average:
            return None
        else:
            return history.iloc[average].name.to_pydatetime()
...
class Portfolio:
    ...
    def deleteEntry(self, id: int) -> None:
        for i in range(id+1, self.num_entries-1):
    ...
    def get_earliest(self):
        earliest = dt.datetime.now().replace(tzinfo=None)
        for id in self.entries:
            cur = self.entries[id].return_earliest(self.average).replace(tzinfo=None)
            if cur == None:
                return None
            elif cur < earliest:
                earliest = cur
